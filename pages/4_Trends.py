...
from ui_helpers import chart_type_selector, player_multiselect, player_selectbox, position_filter, year_slicer
...
with st.sidebar:
    ...
    name_col_side = next((c for c in ["player_display_name", "player_name"] if c in scored.columns), None)
    scored = player_multiselect(
        scored,
        key="trends_players",
        label="Players",
        name_col=name_col_side,
        sidebar=True,
        empty_means_no_filter=True,
    )
...
with tabs[0]:
    ...
    if alerts.empty:
        ...
        with col2:
            alert_sort = alerts.sort_values("value", ascending=False)
            br_ct = chart_type_selector(
                ["Bar", "Scatter", "Horizontal bar"],
                key="breakout_chart",
                label="Chart type",
            )
            color_map = {"target_share": "#1f77b4", "rush_volume": "#ff7f0e"}
            if br_ct == "Bar":
                fig = px.bar(
                    alert_sort,
                    x="player_name",
    ...
    st.caption(
        f"Thresholds: target share ≥ {config.TARGET_SHARE_ALERT:.0%} | rush volume ≥ 12 carries | "
        f"{config.SHORT_WINDOW}-week rolling window"
    )
...
with tabs[1]:
    ...
    if pos_data.empty:
        ...
        if name_col:
            ...
            facet = "season" if len(sel_seasons) > 1 else None
...
with tabs[2]:
    ...
    if xfp.empty or "xfp_delta" not in xfp.columns:
        ...
        if slice_.empty:
            ...
            xfp_ct = chart_type_selector(["Scatter", "Bubble"], key="xfp_chart", label="Chart type")

            with col1:
                plot_slice = slice_.copy()
                if xfp_ct == "Bubble" and "xfp_delta" in plot_slice.columns:
                    plot_slice["_bubble"] = plot_slice["xfp_delta"].abs().clip(lower=0.5)
                    fig = px.scatter(
                        plot_slice,
                        x="xfp",
...
with tabs[3]:
    ...
    if roll_col not in rolled.columns:
        ...
        roll_ct = chart_type_selector(["Scatter", "Bar", "Line"], key="rolling_chart", label="Chart type")
        ...
        with col2:
            if roll_col in slice_.columns and vol_col in slice_.columns:
                plot_df = (
                    slice_[[name_col, roll_col, vol_col, "position"]].dropna()
                    if name_col
                    else slice_[[roll_col, vol_col, "position"]].dropna()
                )
                head_df = plot_df.head(50)
                if roll_ct == "Scatter":
                    fig = px.scatter(
                        head_df,
                        x=roll_col,
                ...
                elif roll_ct == "Bar" and name_col:
                    bar_df = head_df.sort_values(roll_col, ascending=False).head(20)
                    fig = px.bar(
                        bar_df,
                        x=name_col,
                        y=roll_col,
                        color="position",
                        title="Top 20 — avg FP (rolling)",
                        labels={name_col: "Player", roll_col: "Avg FP (rolling)"},
                    )
                    fig.update_layout(xaxis_tickangle=-35)
                elif roll_ct == "Line" and name_col:
                    line_df = head_df.sort_values(roll_col, ascending=False).head(20).reset_index(drop=True)
                    line_df["_rk"] = line_df.index
                    fig = px.line(
                        line_df,
                        x="_rk",
                        y=roll_col,
                        color=name_col,
                        markers=True,
                        title="Top 20 — avg FP (rolling) by rank",
                        labels={"_rk": "Rank (1 = highest)", roll_col: "Avg FP (rolling)", name_col: "Player"},
                    )
                else:
                    fig = px.scatter(
                        head_df,
...
with tabs[4]:
    ...
    if ngs_df.empty:
        ...
        if not available_metrics:
            ...
            filt_row = st.columns(2)
            with filt_row[0]:
                if "position" in ngs_df.columns:
                    p_opts = sorted(ngs_df["position"].dropna().unique().tolist())
                    ngs_pos = st.selectbox("Position", ["All"] + p_opts, key="ngs_pos_filt")
                    if ngs_pos != "All":
                        ngs_df = ngs_df[ngs_df["position"] == ngs_pos]
            with filt_row[1]:
                if ngs_name_col and not ngs_df.empty:
                    picked = player_selectbox(ngs_df, key="ngs_player_dd", label="Player", name_col=ngs_name_col)
                    if picked:
                        ngs_df = ngs_df[ngs_df[ngs_name_col].astype(str) == picked]
            ...
            if group_cols and sel_metric in ngs_df.columns:
                ...
                ngs_primary_ct = chart_type_selector(
                    ["Bar", "Scatter", "Horizontal bar"],
                    key="ngs_primary_chart",
                    label="Chart type",
                )
                ...
                with col2:
                    ...
                    nx = ngs_name_col or group_cols[0]
                    if ngs_primary_ct == "Bar":
                        fig = px.bar(
                            top_plot,
                            x=nx,
                            y=sel_metric,
                ...
                if len(available_metrics) >= 2:
                    ...
                    cmp_ct = chart_type_selector(["Scatter", "Bubble"], key="ngs_cmp_chart", label="Chart type")
                    scatter_agg = ngs_df.groupby(group_cols)[[x_metric, y_metric]].mean().reset_index()
                    scatter_agg["_bubble"] = (
                        np.abs(pd.to_numeric(scatter_agg[x_metric], errors="coerce").fillna(0))
                        + np.abs(pd.to_numeric(scatter_agg[y_metric], errors="coerce").fillna(0))
                    ).clip(lower=0.01)
